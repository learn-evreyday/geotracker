...
async function maybeInsertAlert(deviceId, type, serial, latest) {
  ...
  const bat =
    typeof latest?.batteryPercent === 'number' ? Math.round(latest.batteryPercent) : null;
  const severity =
    type === 'offline'
      ? 'high'
      : bat != null && bat < 10
  ...
  if (type === 'offline') {
    ...
    const pct = bat != null ? `${bat}%` : '?%';
    message =
      bat != null && bat < 10
        ? `Tracker ${serial} has critical battery (${pct}) while offline.`
        : `Tracker ${serial} has low battery (${pct}).`;
  }
...
export async function syncAlertsFromTelemetry() {
  ...
  for (const d of rows) {
    ...
    if (latest.status === 'offline') {
      ...
      // Even when offline is the primary status, still alert on critical battery.
      if (typeof latest.batteryPercent === 'number' && latest.batteryPercent < 10) {
